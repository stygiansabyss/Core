...
class UpdateCoreCommand extends Command {
	 ...
	 * @var string
	 */
	protected $name = 'nuka:update-core';
	 ...
	 * @var string
	 */
	protected $description = 'Perform any steps needed for nuka\core updates.';
	...
	{
		// Handle Core
		if ($this->confirm('Do you want to update nuka\Core? [yes|no]')) {
			...
			$newVersion     = nuka\Core\CoreServiceProvider::version;
	...
	protected function composerUpdate($version)
	{
		$commands = [
			'cd '. base_path(),
			'composer update nuka/core:'. $version,
		];
